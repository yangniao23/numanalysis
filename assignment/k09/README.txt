...
3. `make` コマンドを実行する．
4. 実行ファイル `k10.exe` が生成される．

## 3. 実行方法
1. ビルド方法の手順1~4を実行する．
2. 評価用データファイルを追加する．
    - 実行ファイルと同じディレクトリに配置する．
    - ファイル名は `k10-input.csv`とする．
3. `k10.exe` を実行する．

## 4. 補足
- データファイルのファイル名は `src\k10.c` 中の `VECTOR_FILENAME` で定義されている．
...
```
gcc -O2 -march=native -o k10.exe src\k10.c lib\NAbasic.c -Ilib -Isrc -Llib -lm
```

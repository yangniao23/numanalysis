...
#define PARAM_FILENAME "k13-input.csv"
...
Matrix *heun_method(F f, double x0, double width, double x_inf, double y0) {
    size_t len = (size_t)((x_inf - x0) / width) + 1;  // len = n + 1
    Matrix *matrix = allocMatrix(2, len);  // data[0][i] = x_i, data[1][i] = y_i
    size_t i;
    double *x = matrix->data[0];  // assign x, y to improve readability
    ...
    for (i = 1; i < len; i++) {
        ...
        double k2 = f(x[i - 1] + width, y[i - 1] + width * k1);

        x[i] = x[i - 1] + width;
        y[i] = y[i - 1] + width * (k1 + k2) / 2;
    }
...
int main(void) {
    ...
    printf("step width: %f\n", matrix->data[0][1]);
    res = heun_method(df, matrix->data[0][0], matrix->data[0][1],

...
3. `make` コマンドを実行する．
4. 実行ファイル `k14.exe` が生成される．

## 3. 実行方法
1. ビルド方法の手順1~4を実行する．
2. 評価用データファイルを追加する．
    - 実行ファイルと同じディレクトリに配置する．
    - ファイル名は `k14-input.csv`とする．
3. `k14.exe` を実行する．

## 4. 補足
- データファイルのファイル名は `src\k14.c` 中の `PARAM_FILENAME` で定義されている．
- 被計算関数は `src\k14.c` 中の `double df(double x, double y)` で定義されている．
...
```
gcc -O2 -march=native -o k14.exe src\k14.c lib\NAbasic.c -Ilib -Llib -lm
```
